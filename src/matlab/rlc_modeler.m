function rlc_c0 = rlc_modeler(measured_data,fminmax,init_rlc_c0)
...
%     n = 2;
    ...
    [rlc,~,~,~,~] = lsqnonlin(func_min,rlc0,[0 0 0 0]',[1e3 1e3 1e3 100],options);
    ...
    %     yout(:,2) = imag(R1+1j*wdata*L1+1./(1j*wdata*C1));
        imp = 1j*wdata*1.87e-3+(R1+1j*wdata*L1+1./(1j*wdata*C1))./(1+1j*wdata*C0.*(R1+1j*wdata*L1+1./(1j*wdata*C1)));
